  ...
  return (
    <div className="w-96 bg-white shadow-2xl border-l border-gray-200 overflow-y-auto">
      ...
      <div className="p-4 space-y-4">
        {/* Point Information */}
        <Card className="p-4 border-fiap-pink/20 border-2">
          <h3 className="text-lg font-bold text-fiap-pink mb-2">{point.nome}</h3>
          
          <div className="space-y-2 text-sm">
            <div className="flex items-center space-x-2">
              <span className="font-medium text-fiap-dark">Tipo:</span>
              <span className="capitalize bg-fiap-pink/10 px-2 py-1 rounded text-fiap-pink font-medium">
                {point.tipo}
              </span>
            </div>
            ...
            {point.descricao && (
              <div>
                <span className="font-medium text-fiap-dark">Descrição:</span>
                <p className="text-fiap-gray-dark mt-1">{point.descricao}</p>
              </div>
            )}
            
            {point.horario_funcionamento && (
              <div className="flex items-center space-x-2">
                <Clock className="h-4 w-4 text-fiap-pink" />
                <span className="font-medium text-fiap-dark">Horário:</span>
                <span className="text-fiap-gray-dark">{point.horario_funcionamento}</span>
              </div>
            )}
            
            <div className="flex items-center space-x-2">
              <MapPin className="h-4 w-4 text-fiap-pink" />
              <span className="font-medium text-fiap-dark">Localização:</span>
              <span className="text-fiap-gray-dark">{point.cidade}, {point.uf}</span>
            </div>
            
            {distance && (
              <div className="flex items-center space-x-2">
                <Navigation className="h-4 w-4 text-fiap-pink" />
                <span className="font-medium text-fiap-dark">Distância:</span>
                <span className="text-fiap-gray-dark font-semibold">{distance.toFixed(1)} km</span>
              </div>
            )}
          </div>
        </Card>
        ...
        <div className="space-y-2">
          <Button 
            className="w-full bg-fiap-pink hover:bg-fiap-pink/90 text-white"
          ...
          <Button 
            variant="outline" 
            className="w-full border-fiap-pink text-fiap-pink hover:bg-fiap-pink/10"
            onClick={handleShare}
          >
